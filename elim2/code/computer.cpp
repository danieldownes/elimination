...
void PC_ExecuteMove(void)
{
   // the game is over or the player is not a computer
   if(g_settings & SET_GAME_OVER || !(g_pFlags[g_cColor] & PLAYER_IS_PC)) return;

#ifdef ELIM_DEBUG
   IO_DebugWrite("call: ExecuteMove", 0);
#endif
   ...
   const int dice[] = {g_diceB[0].value + 1, g_diceB[1].value + 1}; // dice values
               ...
               for(l = 0; l < 2; l++)
               {
                  ...
                  if(0 < x && x < 15 && 0 < y && y < 15)
                  {
                     ...
                     if(g_board[x][y] > MARBLE_INVALID)
                     {
                        ...
                        moves[index].free = (g_board[x][y] == MARBLE_FREE);

                        // figure out who i will jump and how many
                        GetJumped(i, j, x, y, &moves[index].nJumped,
                           moves[index].jumped);
   ...
   if(index <= 0)
   {
      Dlg_MsgBox("The Computer was unable to locate any moves", "AI Error", "OK", NULL);
      return;
   }
   ...
   if(g_board[choosen->sx][choosen->sy] != g_cColor)
   {
      Dlg_MsgBox("The AI was trying to move an invalid marble", "AI Error", "OK", NULL);
      return;
   }

   // move the marble
   Marble_Move(choosen->sx, choosen->sy, choosen->dx, choosen->dy);
}
...
int nHit)       // number of moves this marble will hit
{
   ...
   *num  = 0;
   ...
   {
      // clear the locations
      for(i = 0; i < 20; i++) for(j = 0; j < 2; j++) loc[i][j] = D_INVALID;
   ...
   for(i = 0; i < 4; i++)
   {
      for(j = 0; j < 5; j++)
      {
         ...
         if(-1 < x && x < 16 && -1 < y && y < 16)
         {
            ...
            if(Marble_Enemy(g_cColor, g_board[x][y]))
            {
               // record the location
               loc[index][0] = x;
               loc[index][1] = y;
...
static AI_MOVE* Move_Free(AI_MOVE* freMoves[320], int nMoves)
{
   ...
   for(i = 3; i < 12; i++) if(g_board[*x][*y] != MARBLE_SPACE) lineNum++;
